...
class DreamCard extends Component {
  ...
  componentDidMount() {
    this.setState({
      ...
      cohesion: this.props.e.cohesion,
      lucidity: this.props.e.lucidity
    });
  }
  ...
  ratingHandler() {
    axios
      .put(`/api/ratings/${this.props.e.dream_id}`, {
  ...
  render() {
    ...
    return (
      <div className="dreamcard">
        <h8 id="timeStamp">
          Posted on:{moment(this.props.e.date_created).format("LLL")}
        </h8>

        <h3 key={this.props.e.dream_id} onClick={() => this.handleMouseClick()}>
          {this.props.e.dream}
        </h3>
        ...
        {this.state.isClicked && (
          <button className="share">
            <Share onClick={() => this.props.share(this.props.e.dream_id)} />
          </button>
        )}
        <DeleteSharpIcon
          className="deleteButton"
          onClick={() => this.props.delete(this.props.e.dream_id)}
        />
      </div>
    );
  }
