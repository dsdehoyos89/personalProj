...
class PublicV extends Component {
  ...
  async componentDidMount() {
    ...
    await axios
      .get("/api/publicDreams")
      .then(response => this.setState({ publicDreams: response.data }));
  }
  ...
  render() {
    ...
    let buttonToggle = this.props.loggedIn ? (
      <h1 id="conditionalRendIn"> Public View</h1>
    ) : (
    ...
    const dreams = this.state.publicDreams.map((e, i) => {
      return (
        <div className="dreamcard">
          <h8 id="timeStamp">
            Posted on:{moment(e.date_created).format("LLL")}
          </h8>
          <h8 id="userName">@{e.profile_name}</h8>
          <h3 className="dreamcard" key={e.dream_id}>
            {e.dream}
          </h3>
